...
本仓库同时包含 Linux 和 Windows 两套 opencode 入口文件。安装时必须根据当前操作系统选择对应文件，并复制成 opencode 期望的固定文件名：
...
注意：目标项目里最终只需要 `.opencode/skills/reimbursement/SKILL.md` 和 `.opencode/agents/fix-invoice-errors.md`。如果你是 Windows 用户，应把仓库里的 `.windows.md` 文件复制并重命名成上述固定文件名，而不是把 `.windows.md` 作为额外入口文件放进 `.opencode/skills/` 或 `.opencode/agents/`。
