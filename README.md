...
- Linux 版本：使用 `main` 分支，命令使用 `bash`、`source .venv/bin/activate`、`mkdir`、`cp` 等 Linux 写法。
- Windows 版本：使用 `windows-version` 分支，命令使用 PowerShell、`.\.venv\Scripts\python.exe`、`New-Item`、`Copy-Item` 等 Windows 写法。

如果你不确定该用哪个版本：Windows 用户请选择 `windows-version` 分支；Linux 用户请选择 `main` 分支。

## 安装指南（给Agent看的）

当用户要求你安装此报销 skill 时，先确认用户使用的操作系统：

- 如果用户使用 Linux：使用 `main` 分支，克隆本仓库后阅读此节，按以下步骤安装。
- 如果用户使用 Windows：切换到 `windows-version` 分支，阅读该分支 README.md 中的安装指南，并按 Windows/PowerShell 命令安装。
...
> 请帮我安装 Linux 版报销 skill，仓库在 <https://github.com/guanggaofei/reimbursement-opencode-skill>，使用 main 分支，根据 README.md 安装。

Windows 用户说：

> 请帮我安装 Windows 版报销 skill，仓库在 <https://github.com/guanggaofei/reimbursement-opencode-skill>，使用 windows-version 分支，根据 README.md 安装。

#### 方法二：自己手动安装

如果你不想用 agent，也可以直接复制文件：

1. 下载或克隆对应分支：Linux 用户使用 `main` 分支，Windows 用户使用 `windows-version` 分支。
