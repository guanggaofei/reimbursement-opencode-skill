...
**检测关键字：** grep `支出记录OCR匹配明细.md` 查找 `金额对应多个候选发票`
...
**检测关键字：** grep `支出记录OCR匹配明细.md` 查找 `金额对应多个候选行程`
...
**方法：**
1. 全文 grep `支出记录OCR匹配明细.md` 中所有该金额的 pending 图片（支付记录+账单截图都要），不要只抓一组。
...
**检测关键字：** grep `支出记录OCR匹配明细.md` 查找 `同时匹配同一发票，需人工识别`
