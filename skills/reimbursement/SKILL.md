...
合入脚本负责更新 `匹配记录.json`，并强制所有路径使用稳定状态路径：`invoices/<invoice_results_sorted.json 的 文件名 字段值>` 和 `images/<原截图文件名>`。除 `fix-bearing-invoice` 和主流程人工确认外，同一材料费发票、同一打车行程最多只能有一张 `支付记录` 和一张 `账单截图`。如果脚本输出 `ERROR`、非零退出，或 subagent 报告其无法处理的错误信息，立即停止并把错误报告给用户，不要自行猜测修复。
